...
<div class="wrap">
    ...
    <div class="container" dir="ltr">
        <header class="header">
            ...
            <?php settings_errors(); ?>
            <div class="ml-auto">
                <a class="btn-white" href="<?php echo esc_url($widgetURL) ?>">
        ...
        <?php if (count($chatyWidgets)) { ?>
            <div class="chaty-table">
                ...
                <table>
                    <thead>
                        <tr>
                            <th class="fix-width"><?php esc_html_e("Status", 'chaty'); ?></th>
                            <th><?php esc_html_e("Widget name", 'chaty'); ?></th>
                            <?php if ($dateStatus) { ?>
                                <th><?php esc_html_e("Created On", 'chaty'); ?></th>
                            <?php } ?>
                            <th class="fix-width"><?php esc_html_e("Actions", 'chaty'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr id="widget_<?php echo esc_attr($widget['index']) ?>" data-widget="<?php echo esc_attr($widget['index']) ?>" data-nonce="<?php echo esc_attr($widget['nonce']) ?>">
                                ...
                                <?php if ($dateStatus) { ?>
                                    <?php if (!empty($widget['created_on'])) {?>
                                        <td><?php echo date("F j, Y", strtotime($widget['created_on'])) ?></td>
                                    <?php } else { ?>
                                        <td>&nbsp;</td>
                                    <?php } ?>
                                <?php } ?>
        ...
        <?php } else { ?>
            <div class="chaty-table no-widgets">
                <img src="<?php echo CHT_PLUGIN_URL ?>/admin/assets/images/no-widget.png" />
                <div class="no-widget-title"><?php esc_html_e("Create Your First Widget", "chaty") ?></div>
                <div class="no-widget-desc"><?php esc_html_e("Create widgets for WhatsApp, Facebook Messenger, Telegram & 20+ more channels. Adding a new Chaty widget takes a minute - start now  🚀", "chaty") ?></div>
                <div class="no-widget-btn"><a href="<?php echo admin_url("admin.php?page=chaty-app&widget=0") ?>"><?php esc_html_e("Create New Widget", "chaty") ?></a></div>
            </div>
        <?php }//end if
        ?>
    </div>
